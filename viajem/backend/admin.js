/* ============================================================
   AHGA TURISMO - Painel Admin com Múltiplos Usuários
   + Botão "Comprar" nas excursões públicas
   ✅ CORRIGIDO: sem listeners duplicados
   ============================================================ */
...
/* ============================================================
   SESSÃO (usa chave exclusiva 'ahga_sessao')
   ============================================================ */
...
function setSessao(dados) {
  if (dados) localStorage.setItem(SESSION_KEY, JSON.stringify(dados));
...
function mudarAba(e, aba) {
  ...
  if (tab) {
    ...
    setTimeout(() => tab.classList.add('tab-fade-in'), 10);
  }
    ...
    if (!lista.length) {
      container.innerHTML = `
        <div class="empty-state" style="grid-column:1/-1;">
          ...
          <p>Nenhuma excursão publicada ainda.</p>
          <span>Volte em breve para novidades!</span>
        </div>`;
      return;
    }

    container.innerHTML = lista.map(exc => renderExcursaoCard(exc)).join('');
  } catch (err) {
    console.error(err);
    container.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1;">
        <i class="fas fa-exclamation-triangle"></i>
        <p>Erro ao carregar excursões.</p>
      </div>`;
  }
}

/* ============================================================
   RENDER DO CARD (com botão COMPRAR)
   ============================================================ */
function renderExcursaoCard(exc) {
  ...
  const preco = Number(exc.preco).toLocaleString('pt-BR', { minimumFractionDigits: 0 });
        ...
        <div class="excursao-footer">
          ...
          <div class="excursao-acoes">
            ...
            <div style="display:flex; gap:0.4rem; flex-wrap:wrap; justify-content:flex-end;">
              <button class="btn-secondary small" onclick='verDetalhesExcursao("${exc.id}")'>
                <i class="fas fa-info-circle"></i> Detalhes
              </button>
              <button class="btn-primary small" 
                      onclick='abrirCompra("${exc.id}")' 
                      ${semVagas ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : ''}>
...
/* ============================================================
   DETALHES DA EXCURSÃO (modal público)
   ============================================================ */
async function verDetalhesExcursao(id) {
  try {
    ...
    const img = exc.imagem || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1600&q=80';
    const inclui = (exc.inclui || []).map(i => `<li><i class="fas fa-check"></i> ${i}</li>`).join('');
    const roteiro = (exc.roteiro || []).map(r => `<li><i class="fas fa-circle-dot"></i> ${r}</li>`).join('');
    const semVagas = !exc.vagas || exc.vagas <= 0;
    ...
    document.getElementById('modalContent').innerHTML = `
      ...
      ${exc.descricao ? `
        <div class="modal-section">
          ...
          <p style="font-size:0.9rem;color:#6b7d98;line-height:1.6;">${exc.descricao}</p>
        </div>` : ''}

      ${inclui ? `
        <div class="modal-section">
          <h4><i class="fas fa-check-circle"></i> O que está incluso</h4>
          <ul class="included-list">${inclui}</ul>
        </div>` : ''}

      ${roteiro ? `
        <div class="modal-section">
          <h4><i class="fas fa-route"></i> Roteiro</h4>
          <ul class="included-list">${roteiro}</ul>
        </div>` : ''}
      ...
      <div class="modal-actions" style="display:flex; gap:0.6rem; margin-top:1rem;">
        ...
        <button class="btn-primary" style="flex:2;" 
                onclick='closeModal(); abrirCompra("${exc.id}")'
                ${semVagas ? 'disabled style="flex:2;opacity:0.5;cursor:not-allowed;"' : ''}>
    ...
    if (!lista.length) {
      container.innerHTML = `
        <div class="empty-state">
          ...
          <p>Nenhuma excursão cadastrada.</p>
          ${sessao.podeCriar ? `<span>Clique em "Nova excursão" para começar.</span>` : ''}
        </div>`;
      return;
    }
          ...
          ${lista.map(exc => `
            <tr>
              ...
              <td class="admin-actions-cell">
                ${sessao.podeEditar ? `
                  <button class="icon-btn edit" onclick='editarExcursao("${exc.id}")' title="Editar">
                    <i class="fas fa-pen"></i>
                  </button>` : ''}
                ${sessao.podeExcluir ? `
                  <button class="icon-btn delete" onclick='excluirExcursao("${exc.id}", "${exc.titulo.replace(/"/g, '&quot;')}")' title="Excluir">
...
function abrirFormExcursao(exc = null) {
...
/* ============================================================
   ✅ INICIALIZAÇÃO ÚNICA E CORRIGIDA
   ============================================================ */
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 admin.js inicializado');
  atualizarHeader();

  /* ---------- LOGIN ADMIN (único listener) ---------- */
  ...
  if (formLogin) {
    formLogin.addEventListener('submit', (e) => {
      ...
      if (conta && conta.senha === senha) {
        ...
        console.warn('❌ Credenciais inválidas para:', user);
        mostrarErro('Usuário ou senha incorretos.');
      }
    });
  } else {
    console.error('❌ formLoginAdmin NÃO encontrado no HTML!');
  }

  /* ---------- FORM EXCURSÃO ---------- */
  const formExc = document.getElementById('formExcursao');
  if (formExc) {
    formExc.addEventListener('submit', async (e) => {
      e.preventDefault();
      const sessao = getSessao();
      if (!sessao) {
  ...
  document.getElementById('excursaoModal')?.addEventListener('click', (e) => {
