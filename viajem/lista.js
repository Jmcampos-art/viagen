/* ============================================================
   ...
   DUAS VISUALIZAÇÕES:
   1. 🚌 Lista de Ônibus (Nome + RG + Check-in)
...
let abaListaAtual = 'onibus';
let checkinsDaLista = new Set();
...
async function abrirListaExcursao(excursaoId) {
  ...
  try {
    ...
    // ✅ Carrega check-ins e inicia monitoramento em tempo real
    await carregarCheckinsDaExcursao(excursaoId);
    if (typeof iniciarMonitorCheckin === 'function') {
      iniciarMonitorCheckin(excursaoId);
    }
...
/* ============================================================
   ✅ CARREGAR CHECK-INS (quem já embarcou)
   ============================================================ */
async function carregarCheckinsDaExcursao(excursaoId) {
  try {
    const r = await fetch(`${API_URL_LISTA}/api/compras/excursao/${excursaoId}/checkins`);
    const data = await r.json();
    const lista = data.checkins || [];
    checkinsDaLista = new Set(lista.map(c => c.codigo));
  } catch (err) {
    console.warn('Erro ao carregar check-ins:', err);
    checkinsDaLista = new Set();
  }
}
...
/* ============================================================
   ABA 1: LISTA DO ÔNIBUS (Nome + RG + ✅ Check-in)
   ============================================================ */
function renderAbaOnibus(compras) {
  const totalEmbarcados = compras.filter(c => checkinsDaLista.has(c.codigo)).length;

  let html = `
    <div class="lista-onibus-aviso">
      <i class="fas fa-bus"></i>
      Confira os passageiros no embarque.
      Total: <strong>${compras.length}</strong> comprador(es) ·
      ✅ <strong>${totalEmbarcados}</strong> embarcado(s)
    </div>
    ...
    <table class="lista-tabela lista-tabela-onibus">
      ...
      <tbody>
        ${compras.map((c, i) => {
          const embarcou = checkinsDaLista.has(c.codigo);
          return `
            <tr data-checkin-codigo="${c.codigo}" class="${embarcou ? 'passageiro-embarcado' : ''}">
              <td>${i + 1}</td>
              <td>
                <strong>${c.nome || '—'}</strong>
                ${embarcou ? '<span class="nome-check-ok">✅</span>' : ''}
              </td>
              <td>${c.rg || '—'}</td>
              <td>${c.qtd || 1}</td>
              <td>
                <span class="checkin-badge ${embarcou ? 'ok' : 'pendente'}">
                  ${embarcou ? '<i class="fas fa-check"></i> Embarcado' : '<i class="fas fa-clock"></i> Pendente'}
                </span>
              </td>
            </tr>
          `;
...
function copiarListaOnibus() {
  ...
  compras.forEach((c, i) => {
    const embarcou = checkinsDaLista.has(c.codigo);
    texto += `${i + 1}. ${c.nome} | RG: ${c.rg} | Qtd: ${c.qtd || 1}${embarcou ? ' ✅' : ''}\n`;
  });
  ...
  texto += `🎫 *Embarcados: ${totalEmbarcados}/${compras.length}*`;
...
function fecharLista() {
  ...
  if (typeof pararMonitorCheckin === 'function') {
    pararMonitorCheckin();
  }
}
...
console.log('✅ lista.js carregado (2 abas: ônibus + fechamento + check-in visual)');