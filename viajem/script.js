...
async function aiGenerateItinerary() {
  ...
  try {
    console.log(`🤖 Pedindo recomendações para a IA... (já usados: ${sessionUsedDestinations.size})`);
    
    const excludeList = Array.from(sessionUsedDestinations);
    console.log(`📋 Excluindo: ${excludeList.join(', ') || '(nenhum)'}`);
    ...
    const recommendationsFiltradas = recommendations.filter(r => {
      const jaUsado = sessionUsedDestinations.has(r.name);
      if (jaUsado) console.log(`⚠️ IA sugeriu "${r.name}" que já foi usado. Filtrando...`);
      return !jaUsado;
    });

    const finalRecs = recommendationsFiltradas.length > 0 ? recommendationsFiltradas : recommendations;

    console.log(`✅ IA recomendou ${finalRecs.length} destinos novos`);
    ...
    console.log(`📝 Sessão agora tem ${sessionUsedDestinations.size} destinos usados`);
...
function renderItinerary(itinerary) {
  ...
  if (itinerary.dateValue && itinerary.dateEndValue) {
    const start = new Date(itinerary.dateValue + 'T00:00:00');
    const end = new Date(itinerary.dateEndValue + 'T00:00:00');
    const monthsShort = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
    const nights = calculateNights(itinerary.dateValue, itinerary.dateEndValue);
  ...
  itinerary.destinations.forEach(dest => {
    ...
    if (itinerary.dateValue && itinerary.dateEndValue) {
      ...
      dates = getBookingDates(dest.idealDays);
    }
...
function openModal(id) {
  ...
  if (dateValue && dateEndValue) {
    dates = { checkin: dateValue, checkout: dateEndValue };
  } else {
    dates = getBookingDates(nights);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('dateEndInput')?.addEventListener('change', updateNightsInfo);
  
  // ✅ Auto-preenche data de saída
  ...
  // ✅ Valida: data saída deve ser depois da entrada
  document.getElementById('dateEndInput')?.addEventListener('change', (e) => {
    const startValue = document.getElementById('dateInput').value;
    if (startValue && e.target.value && e.target.value < startValue) {
      alert('⚠️ A data de saída deve ser depois da data de entrada!');
      e.target.value = '';
      updateNightsInfo();
    }
  });
