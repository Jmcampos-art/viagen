...
/* ---------- BUSCA COM IA (destino específico) ---------- */
async function searchWithAI(query) {
  ...
  const destination = {
    ...
    image: data.image || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1600&q=80',
...
async function filterDestinations() {
  ...
  renderCards(filtered, false, originText, nights, budgetRaw);
}

/* ---------- RENDERIZAÇÃO DOS CARDS ---------- */
function renderCards(list, isAIPick = false, originText = '', nights = 7, budgetRaw = 0) {
  ...
  const partes = [];
  if (nights) partes.push(`🌙 ${nights} noites`);
  if (budgetRaw > 0) partes.push(`💰 R$ ${budgetRaw.toLocaleString('pt-BR')}`);
  ...
  list.forEach(dest => {
    ...
    const destNights = nights || dest.idealDays;
    const hotelTotal = calculateHotelTotal(dest.pricing, destNights);
      ...
      <div class="card">
        ...
        <div class="card-content">
          ...
          <div class="details">
            ...
            <div class="detail-item"><i class="fas fa-clock"></i> ${destNights} dias ideais</div>
          ...
          <div class="price-breakdown">
            ...
            <div class="price-row">
              <span><i class="fas fa-hotel"></i> ${destNights} noites</span>
  ...
  container.querySelectorAll('.card-img').forEach(div => {
    ...
    if (match && match[1]) {
      const img = new Image();
      img.onerror = () => {
        div.style.backgroundImage = `linear-gradient(0deg,#00000060,#00000020), url('https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1600&q=80')`;
      };
      img.src = match[1];
    }
  });
  ...
  container.querySelectorAll('[data-action]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (action === 'details') openModalByName(name);
      else if (action === 'packages') openPackagesByName(name);
    });
  });
}
...
function openPackagesByName(name) {
  ...
  if (!dest) { alert('Destino não encontrado: ' + name); return; }
...
function openPackages(destId) {
  ...
  if (!dest) { alert('Erro: destino não encontrado.'); return; }
...
function generateOffers(dest) {
  ...
  if (dest.pricing.flight !== null && dest.pricing.flight !== undefined) {
    ...
    airlinesList.forEach(airline => {
      ...
      offers.push({
        id: ++counter, type: 'flight', airline,
        ...
        rating: airline.rating, isBestPrice: false
      });
    });
  }
  ...
  shuffledHotels.forEach(hotel => {
    ...
    offers.push({
      id: ++counter, type: 'hotel', chain: hotel,
      name: `${hotel.name} · ${dest.name}`,
      price: pricePerNight, oldPrice, nights,
      tags: [`${nights} noites`, `R$ ${pricePerNight.toLocaleString('pt-BR')}/noite`, hotel.type],
      rating: hotel.rating, isBestPrice: false
    });
  });
  ...
  packageProviders.forEach(provider => {
    if (dest.pricing.flight !== null && dest.pricing.flight !== undefined) {
      ...
      offers.push({
        ...
        rating: (7.5 + Math.random() * 2).toFixed(1), isBestPrice: false
      });
    }
...
function renderItinerary(itinerary) {
  ...
  if (itinerary.nights) {
    ...
    if (itinerary.dateValue) {
      ...
      monthLabel = `${d.getDate()} ${monthsShort[d.getMonth()]} · ${itinerary.nights}n`;
    }
  }
  ...
  summary.innerHTML = `
    ...
    <div class="summary-card"><i class="fas fa-clock"></i><span class="label">Duração</span><span class="value">${itinerary.totalDays} dias</span><span class="sub">${itinerary.nights ? `${itinerary.nights} noites` : 'Roteiro dia a dia'}</span></div>
  ...
  timeline.querySelectorAll('.timeline-item-img').forEach(div => {
    ...
    if (match && match[1]) {
      const img = new Image();
      img.onerror = () => {
        div.style.backgroundImage = `linear-gradient(0deg,#00000070,#00000020), url('https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1600&q=80')`;
      };
      img.src = match[1];
    }
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const aiBtnFull = document.getElementById('aiBtnFull');
  if (aiBtnFull) aiBtnFull.addEventListener('click', runAISearch);

  const regenerateBtn = document.getElementById('regenerateBtn');
  if (regenerateBtn) regenerateBtn.addEventListener('click', runAISearch);

  const backBtn = document.getElementById('backBtn');
  if (backBtn) backBtn.addEventListener('click', () => showScreen(null));

  const backFromPackages = document.getElementById('backFromPackages');
  if (backFromPackages) backFromPackages.addEventListener('click', () => showScreen('resultsScreen'));

  const backFromItinerary = document.getElementById('backFromItinerary');
  if (backFromItinerary) backFromItinerary.addEventListener('click', () => showScreen(null));
...
console.log('✅ AHGA Turismo script.js carregado!');